...
dnf install bash-completion man-pages rpm-build bzip2-devel createrepo subscription-manager mlocate tuned sysstat curl wget net-tools vim lrzsz tree screen lsof tcpdump nc mtr nmap\
		conntrack ipvsadm ipset jq  iptables iptables-services nfs-utils bind bind-utils openssl openssl-devel readline-devel libseccomp libffi-devel
```
